...
public class VerticalHeader {
  ...
  public VerticalHeader(@NonNull Activity activity,
      @NonNull HeaderAndFooterRecyclerView recyclerView) {
    this.recyclerView = recyclerView;
    headerView = LayoutInflater.from(activity)
        .inflate(R.layout.header_vertical, recyclerView.getHeaderContainer(), false);
    recyclerView.addHeaderView(headerView);
    ButterKnife.bind(this, headerView);
    tvName.setBackgroundColor(RandomUtils.randomColor());
  }

  @OnLongClick(R.id.btn_item) boolean onBtnItemLongClick() {
    recyclerView.removeHeaderView(headerView);
    return true;
  }

  @OnClick(R.id.btn_item) void onClick() {
    setText("header ++ ");
  }
